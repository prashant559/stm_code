    ...
    if (nand_cyclic_run) {
        ...
        for (int block = start_block; block <= end_block; block++) {
            for (int page = 0; page <= MAX_PAGE; page++) {
                ...
                HAL_UART_Transmit(&huart5, read_data, sizeof(read_data),0xffffff);
            }
        }

        for (int block = 0; block <= nand_address.Block; block++) {
            for (int page = 0; page <= MAX_PAGE; page++) {
                nand_read_address.Block = block;
                nand_read_address.Page = page;
                nand_read_address.Plane = 0;
