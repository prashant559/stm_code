...
/*
 void BootloaderMain(void) {
 uint32_t flashStartAddress = 0x08008000;  // Flash address for application
 //    uint8_t buffer[256];                      // Buffer for received data
 uint8_t *buffer;
 uint32_t remainingSize = 59392;
 uint32_t timeout = 0xFFFFFFFF;  // 5-second timeout for UART receive
 ...
 HAL_UART_Transmit(&huart5, (uint8_t *)readyMessage, strlen(readyMessage), HAL_MAX_DELAY);

 // Receive the file size (4 bytes)
 //    HAL_StatusTypeDef status = HAL_UART_Receive(&huart5, (uint8_t *)&remainingSize, sizeof(remainingSize), timeout);
 //    if (status != HAL_OK) {
 //        printf("UART receive error or timeout\n");
 //        return;  // Exit if there is an error receiving the file size
 //    }
 ...
 HAL_UART_Transmit(&huart5, (uint8_t *)sendMessage, strlen(sendMessage), HAL_MAX_DELAY);

 // Receive and program the file in chunks
 while (remainingSize > 0) {
 uint32_t chunkSize = (remainingSize > sizeof(buffer)) ? sizeof(buffer) : remainingSize;
 ...
 // Ensure the buffer is aligned to 32-bit (4-byte) boundary
 if ((uintptr_t)buffer % 4 != 0) {
 // Handle misalignment (you can adjust the buffer or allocate aligned memory)
 uint32_t alignedBuffer[64]; // 32-bit aligned buffer
 buffer = (uint8_t *)alignedBuffer;  // Use the aligned buffer
 }

 // Receive a chunk of data
 uint8_t status = HAL_UART_Receive(&huart5, buffer, 256, timeout); //chunkSize
 if (status != HAL_OK) {
 printf("UART receive error or timeout\n");
 return;  // Exit on timeout or error
 }
 ...
 HAL_UART_Transmit(&huart5, (uint8_t *)successMessage, strlen(successMessage), HAL_MAX_DELAY);
...
/*
 ...
 // Receive a chunk of data
 HAL_UART_Receive(&huart5, buffer, chunkSize, HAL_MAX_DELAY);
